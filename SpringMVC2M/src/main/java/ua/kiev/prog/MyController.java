...
public class MyController {
    ...
    @RequestMapping("/list")
    public String onGetList(Model model){
        model.addAttribute("photos", photos.keySet());
        return "list";
    }

    @RequestMapping(value = "/deletelist", method = RequestMethod.POST)
    public String onDeleteList(Model model, @RequestParam("checked_photo") String par) {
        System.out.println(par);
        for(String x : Arrays.asList(par.split(","))){
            Long id = Long.parseLong(x);
            onDelete(id);
        }
        return onGetList(model);
    }
